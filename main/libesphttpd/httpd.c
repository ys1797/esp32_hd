...
//Start the response headers.
void ICACHE_FLASH_ATTR httpdStartResponseEx(HttpdConnData *conn, int code, const char *reason)
{
	char buff[256];
	int l;
	const char *connStr="Connection: close\r\n";
	if (conn->priv->flags&HFL_CHUNKED) connStr="Transfer-Encoding: chunked\r\n";
	if (conn->priv->flags&HFL_NOCONNECTIONSTR) connStr="";
	l=snprintf(buff, sizeof(buff), "HTTP/1.%d %d %s\r\nServer: esp32-httpd/"HTTPDVER"\r\n%s", 
			(conn->priv->flags&HFL_HTTP11)?1:0, 
			code, reason? reason:"OK", connStr);
	httpdSend(conn, buff, l);
}
